...
typedef struct {
	...
	unsigned char iv[PTW_IVBYTES];
	// The keystream used in this session
	unsigned char keystream[PTW_KSBYTES];
...
typedef struct {
	...
	unsigned char seen_iv[PTW_IVTABLELEN];
...
int PTW_addsession(PTW_attackstate *, unsigned char *, unsigned char *, int *, int);
int PTW_computeKey(PTW_attackstate *, unsigned char *, int, int, int *, int [][PTW_n], int attacks);
